package com.Amtrak.ICE.components
{
	...
	public class ICEVideoPlayer extends Sprite
	{
		...
		public function loadCaption(src:String):void
		{
			...
			my_FLVPlybkcap.addEventListener(CaptionChangeEvent.CAPTION_CHANGE, captionChangeHandler);
			my_FLVPlybkcap.source = src;
			//my_FLVPlybkcap.showCaptions = false;
			my_FLVPlybkcap.visible = false;
			my_FLVPlybkcap.alpha = 0;
			
		}
		public function setMute(mute:Boolean):void
		{
			
			var muteBtn:MovieClip = vol.muteButton;
			if (mute)
			{
		...
		private function captionChangeHandler(e:CaptionChangeEvent):void 
		{
			tf = e.target.captionTarget;
			//trace(tf.text);
            var player:FLVPlayback = e.target.flvPlayback;
            // move the caption below the video
            tf.y = 210;        
			tf.visible = false;
			dispatchEvent(new ICETimedTextEvent(ICETimedTextEvent.CAPTION_UPDATED, tf));
		}
		private function captionTargetCreatedHandler(e:CaptionTargetEvent):void
		{
			...
			(e.captionTarget as TextField).visible = false;
			(e.captionTarget as TextField).alpha = 0;
			//my_FLVPlybkcap.showCaptions = false;
