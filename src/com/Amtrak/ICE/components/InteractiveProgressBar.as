package com.Amtrak.ICE.components
{
	...
	import mx.events.*;
	public class InteractiveProgressBar extends Sprite
	{
		...
		public function InteractiveProgressBar()
		{
			_dataSource = null;
			_selectedNode = null;
			parentClip = null;
		...
		private function onXMLChange(e:PropertyChangeEvent):void
		{
			...
			buildDataTree(_dataSource[0].children()[1], _tree, counter);
		...
		private function buildDataTree(xml:XML, arr:Array, counter:int):void
        {
			...
			for each(var n:XML in nodeList)
			{
				...
				if(n.hasOwnProperty("node-collection"))
				{
					node.isBranch = true;
					node.expanded = false;
					buildDataTree(n.children()[1], node.children, counter);
				}
				count++;
			}
            return;
