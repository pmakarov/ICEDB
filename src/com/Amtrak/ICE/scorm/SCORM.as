	...
	public class SCORM {
		...
		public function SCORM() {
			...
			if(is_EI_available){
				...
				wrapperFound = Boolean(ExternalInterface.call("ICE.SCORM.isAvailable"));
		...
		public function connect():Boolean {
			__displayDebugInfo("ICE.SCORM.connect() called from class file");
			return __connect();
		}
		...
		private function __connect():Boolean {
			...
			if(!__connectionActive){
				...
				var eiCall:String = String(ExternalInterface.call("ICE.SCORM.init"));
				...
				if (result){
					...
					if(errorCode){
						...
						__displayDebugInfo("ICE.SCORM.init() failed. \n"
										  +"Error code: " +errorCode +"\n"
										  +"Error info: " +debugInfo);
					} else {
						__displayDebugInfo("ICE.SCORM.init failed: no response from server.");
					}
				}
			} else {
				  __displayDebugInfo("ICE.SCORM.init aborted: connection already active.");
			}
		...
		private function __disconnect():Boolean {
			...
			var result:Boolean = false;
			if(__connectionActive){
				var eiCall:String = String(ExternalInterface.call("ICE.SCORM.quit"));
				result = __stringToBoolean(eiCall);
				if (result){
					...
					var errorCode:int = __getDebugCode();
					var debugInfo:String = __getDebugInfo(errorCode);
					__displayDebugInfo("ICE.SCORM.quit() failed. \n"
									  +"Error code: " +errorCode +"\n"
									  +"Error info: " +debugInfo);
				}
			} else {
				__displayDebugInfo("ICE.SCORM.quit aborted: connection already inactive.");
			}
			return result;
		}
		...
		private function __get(parameter:String):String {
			...
			if (__connectionActive){
				...
				returnedValue = String(ExternalInterface.call("ICE.SCORM.get", parameter));
				var errorCode:int = __getDebugCode();
				...
				if (returnedValue == "" && errorCode != 0){
					var debugInfo:String = __getDebugInfo(errorCode);
					__displayDebugInfo("ICE.SCORM.get(" +parameter +") failed. \n"
									  +"Error code: " +errorCode +"\n"
									  +"Error info: " +debugInfo);
				}
			} else {
				__displayDebugInfo("ICE.SCORM.get(" +parameter +") failed: connection is inactive.");
			}		
		...
		private function __set(parameter:String, value:String):Boolean {
			...
			var result:Boolean = false;
			if (__connectionActive){
				var eiCall:String = String(ExternalInterface.call("ICE.SCORM.set", parameter, value));
				result = __stringToBoolean(eiCall);
				if(!result){
					var errorCode:int = __getDebugCode();
					var debugInfo:String = __getDebugInfo(errorCode);
					__displayDebugInfo("ICE.SCORM.set(" +parameter +") failed. \n"
									  +"Error code: " +errorCode +"\n"
									  +"Error info: " +debugInfo);
				}
			} else {
				__displayDebugInfo("ICE.SCORM.set(" +parameter +") failed: connection is inactive.");
			}
			return result;
		}
		...
		private function __save():Boolean {
			...
			var result:Boolean = false;
			if(__connectionActive){
				var eiCall:String = String(ExternalInterface.call("ICE.SCORM.save"));
				result = __stringToBoolean(eiCall);
				if(!result){
					var errorCode:int = __getDebugCode();
					var debugInfo:String = __getDebugInfo(errorCode);
					__displayDebugInfo("ICE.SCORM.save() failed. \n"
									  +"Error code: " +errorCode +"\n"
									  +"Error info: " +debugInfo);
				}
			} else {
				__displayDebugInfo("ICE.SCORM.save() failed: API connection is inactive.");
			}
			return result;
		}
		...
		private function __getDebugCode():int {
			var code:int = int(ExternalInterface.call("ICE.SCORM.debug.getCode"));
			return code;
		}
			
		private function __getDebugInfo(errorCode:int):String {
			var result:String = String(ExternalInterface.call("ICE.SCORM.debug.getInfo", errorCode));
			return result;
		}
		
		private function __getDiagnosticInfo(errorCode:int):String {
			var result:String = String(ExternalInterface.call("ICE.SCORM.debug.getDiagnosticInfo", errorCode));
			return result;
		}
		...
		private function __displayDebugInfo(msg:String):void {
			if(__debugActive){
				trace(msg);
				ExternalInterface.call("ICE.UTILS.trace", msg);
			}
		}
